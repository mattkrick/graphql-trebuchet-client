...
class GQLTrebuchetClient {
    ...
    dispatch(message) {
        ...
        if (casualOrderingGroup) {
            const previousOperation = Object.keys(this.operations).find((id) => id < opId &&
            ...
            if (previousOperation) {
                ...
                const messagesToCheck = this.pendingMessages.filter((message) => this.operations[message.id].payload.cacheConfig?.metadata?.casualOrderingGroup ===
                    casualOrderingGroup);
                const messageOpIds = messagesToCheck.map((message) => Number(message.id));
                const nextMessageId = String(Math.min(...messageOpIds));
                const messageIdxToFlush = this.pendingMessages.findIndex((message) => message.id === nextMessageId);
